...
## 2026-09-20 馬來西亞沒有香蕉與芒果
- 情況：三國共同的 12 種作物裡，馬來西亞的來源缺兩種。PriceCatcher 的濕巴剎在過去 60 天內**一筆香蕉都沒有回報**（只有超市、迷你市場報），官方品項表裡也**沒有生鮮芒果**（只有芒果汁）。
- 決定：馬來西亞不收這兩種，只補上茄子（TERUNG BULAT，52 個縣有報價）。香蕉與芒果的「各國參考價」卡片只會列台灣與印度兩國。
- 理由：要有這兩種就得收超市價，會破壞「零售＝地區內各濕巴剎報價的中位數」的定義（docs/06 §1.5）；混不同通路的價格比少一個國家更難解釋。
- 影響：`backend/app/seed/MY.yaml`、docs/06 §7.4；跨國卡片在這兩種作物上少一列。
## 2026-09-20 新聞摘要用該國自己的語言（ms、hi）
- 情況：摘要原本只有兩種語言（台灣繁中，印度與馬來西亞英文），但介面已經有 Bahasa Melayu 與 हिन्दी，馬來文或印地文的使用者讀到的是英文摘要。一則新聞只做一份摘要（額度就是這樣算的），不可能每個語言一份。
- 決定（使用者 2026-09-20）：
  - `sources.yaml` 的 `summary_lang` 改成該國自己的語言：馬來西亞 `ms`、印度 `hi`，台灣仍是 `zh-TW`。搜尋用的語言不變（馬來西亞仍然搜英文與馬來文兩個版本）。
  - **長度**：繁中 90 字、英文 50 詞、馬來文 40 詞、印地文 160 個天城體字元。馬來文的詞比英文長約四分之一，所以 40 詞的字數和英文 50 詞差不多；印地文改成數字元（模型對非拉丁文字數字元比數詞準），160 字元在 240×320 的畫面寬度上和繁中 90 字差不多，兩句話放得進新聞卡與內容頁，字級不縮。
  - **語言檢查**（`_language_ok`）：印地文要 20 個以上天城體字元、而且天城體不少於拉丁字母（模型被要求印地文卻回英文很常見，這種要算「沒有摘要」）。馬來文和英文都是拉丁字母，字元集分不出來，所以用**詞彙檢查**：至少出現兩個不同的常見馬來文虛詞（adalah、akan、dan、dari、dengan、di、harga、ialah、ini、itu、juga、ke、kerana、lebih、pada、sudah、tidak、untuk、yang）。一個不算，因為英文句子裡的 Dan、Di 可能是人名；兩個一起出現的英文句子幾乎不存在。檢查不過就不存摘要，只顯示標題。
  - 另一個做法是用語言偵測套件（langdetect、fasttext）：多一個依賴、模型檔要下載，兩句話的偵測也不見得比虛詞檢查準，所以不採用。
  - **畫面**：`summary_lang` 和介面語言不同時，每個畫面用一行寫出摘要的語言（`news.summaryIn.*`，四個語言都有）；清單放在卡片上方一行（128×160 不畫，卡片本來就不畫摘要），內容頁接在「AI 依原文摘要」前面。
- 理由：馬來西亞與印度的讀者大多讀得懂自己的語言，摘要用當地語言比用英文有用；只有一份摘要時，誠實標出它的語言比假裝它是介面語言好。
- 影響：`backend/app/ingest/news/{summarize.py,sources.yaml,config.py,demo.yaml}`、`frontend/src/screens/news/{newsItems.ts,NewsListScreen.tsx,NewsDetailScreen.tsx,news.module.css}`、四個語言檔、`frontend/src/test/fixtures/news__*`；docs/00 決定表、02 §5.9、06 §1.6。已知限制：馬來文與印尼文的虛詞很像，這個檢查分不出兩者。

## 2026-09-20 新聞的作物標籤：寧缺勿錯
- 情況：真實的標題「不是香蕉芒果，1水果大降價！產地價格暴跌68.6%」講的是韓國的香蕉葡萄（葡萄品種），卻被標成香蕉。原因是中文用子字串比對（本來就該這樣，中文沒有空格），而且 `summarize_pending` 把比對器和模型挑的作物取**聯集**，模型沒挑也留著比對器的錯誤。使用者按 `1` 會打開錯誤作物的行情，比沒有標籤更糟。
- 決定：
  1. **否定詞**：同一句之內、作物名稱前幾個字有 不是／並非／而非／不含／不加／除了／沒有，或英文前三個詞有 not／no，這一次比對不算。。！？；：: ; 與換行算一句的結尾（所以「不是香蕉！甘藍價格…」還是標甘藍）；、和，不算（所以「不是香蕉、芒果」兩個都不標）。只對**作物與地區**做，`keywords`／`topics`／`price_words`／`exclude` 不做：它們只決定這則算不算價格新聞，「沒有蔬菜漲價」仍然是價格新聞。`非` 字單用不列入，否則南非、非洲會變成否定詞。
  2. **長詞優先**：比對到的詞落在同一位置的另一個更長的詞裡面時，短的不算（薑在薑黃裡、kubis 在 kubis bunga 裡、Taipei 在 New Taipei 裡）。這一道自動從名稱清單推出來，不用設定；後面又單獨提到短詞時照樣算（「薑黃與薑」兩個都標）。
  3. **`confusable`**：不是作物名稱、卻包含作物名稱的長詞，按國家列在 `sources.yaml`：台灣 `香蕉葡萄`、馬來西亞 `kelapa sawit`、`minyak sawit`。查過三國的作物名稱，只有薑／薑黃與 kubis／kubis bunga 互包，已被第 2 道處理；蔥、茄這種單字不是任何國家的名稱，比對不到。
  4. **模型的作物取代比對器的**（不是聯集）：有摘要的那一則，作物標籤完全來自模型（它讀了原文，而且回答被 JSON Schema 限制在該國的作物代號裡）；稍後才抽到摘要的一則會順便修正之前標錯的作物。代價是模型不確定、一個也不挑時這則就沒有作物標籤，這是刻意的取捨。
  5. **沒有原文也要標作物**：連結解不開或網頁讀不出正文時，改成只問作物的一次模型呼叫（`tags_only`：同一張 JSON Schema、只有 `crops` 欄位；提示詞直接寫 Do not write a summary）。**這種呼叫絕對不產生摘要**，模型自己寫了 `summary` 也會被丟掉。一則一次呼叫，算進 `model_calls` 也算進 `summary_tries`（失敗的一則不會一直重試）。只在這一次執行真的讀不到原文時做；只是「讀原文的額度用完」就不做，那則等下一次執行。`GEMINI_API_KEY` 空的時候關鍵字比對仍然是唯一的標籤來源。
  6. **提示詞**：從「文章主要在講的作物」改成「文章真的報導了價格、產量或供應的作物」，並明寫不要挑只是順口提到的、或標題說「不是」的作物。
- 理由：標錯作物會把使用者帶到錯的行情頁，比沒有標籤傷害大。否定詞與長詞優先便宜又可測，模型取代比對器把最終判斷交給讀過原文的那一方。
- 影響：`backend/app/ingest/news/{match.py,config.py,sources.yaml,summarize.py,pipeline.py}`、`backend/tests/news/{test_match.py,test_pipeline.py,test_summarize.py}`；docs/06 §1.6。

## 2026-09-20 新聞的每日額度從 30 提高到 60
- 情況：使用者查了 `gemini-3.5-flash-lite` 的免費層真實上限：每分鐘 15 次、每天 500 次。原本的 30/30 是保守猜的，而 28 天的高峰只有每天 26 次。
...
- 決定：
  ...
  - **地區＝縣**：馬哈拉施特拉邦、卡納塔卡邦、德里（NCT）中，60 天內至少有一個市場回報這 21 種作物的縣，共 64 個；**市場＝這些縣裡有回報的全部市場**，共 429 個（沒有回報的市場不放進 seed，否則每個作物的市場清單都會多出永遠「無資料」的列）。保留舊代號 nashik、pune、ahmednagar（顯示名稱改成官方的 Ahilyanagar）、jalgaon、solapur、bengaluru（Bengaluru Urban）、kolar、delhi；indore、kurnool、agra 因為不在這三個邦而移除，chintamani 市場改掛在它真正所屬的 Chikkaballapur 縣。清單順序：先邦、再依名稱。
  ...
  - **作物 21 種**：先放三國共同的核心作物（番茄、甘藍、青辣椒、大蒜、洋蔥、馬鈴薯、胡蘿蔔、薑、香蕉、芒果、黃瓜、茄子；lead 為了跨國比較的卡片要求，12 種都有現成圖示），其餘 9 格依 2026-08-01–09-19 的涵蓋度（有回報的「縣 × 交易日」數）補上，並讓每個分類至少 2 種：小麥、高粱、玉米（穀物）、鷹嘴豆、綠豆（豆類）、花生、芝麻（油籽）、棉花、粗糖（其他）。因此原本的大豆、葡萄、稻米、石榴、薑黃、芥菜籽、甘蔗不在清單裡（甘蔗在這三個邦完全沒有回報）；椰子（卡納塔卡邦以每千顆計價、和馬哈拉施特拉邦差 6 倍）與檳榔（等級差 3 倍以上）涵蓋度夠但不收。芒果的涵蓋度低（9 月不是產季，只有 15 個縣），是為了共同作物才留。預設關注把大豆換成鷹嘴豆（涵蓋度第 4 名），其餘不變。新畫 3 個圖示：高粱、綠豆、粗糖（另外畫好的蘋果圖示這次沒用到，留在圖示集裡）。
  ...
  - **User-Agent**：這個 API 對一般函式庫的預設名稱（`python-httpx/…`）回 403，所以每個請求都附上說明自己是誰的 `User-Agent` 與 `Accept: application/json`。
