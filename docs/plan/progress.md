...
- 2026-09-20 追加：資料來源的共同介面與馬來西亞（使用者同意）：每個來源宣告 `SourceInfo` 並登記在 `registry.py`，連網來源共用抓取原則（重啟 6 小時內略過、只抓缺的日期加最近 3 天、檔案沒變用 304）與契約測試；馬來西亞 11 個縣、7 個 Borong 市場、20 種作物（新畫 8 個圖示），示範資料用 PriceCatcher 格式；`my_pricecatcher` 串接 KPDN PriceCatcher，零售＝各縣濕巴剎的中位數，Borong 2026 年起沒有回報所以批發是「—」；本機實跑 3 個請求、約 24 秒、6.4 萬列對照得到，之後的排程 2 個 304，抽查 12 個地區價與原始 CSV 相同；rebase 到 main（5fe6eb6，含 B5）後 make lint、make test（前端 534、後端 354）、make e2e（194 項）全部通過，實跑結果相同；預設仍是 mock，伺服器尚未開啟（f755ce7）
- 2026-09-20 追加：「附近最高／最低」改成統一 100 km 內、同一交易日的所有地區；測試改用台灣的例子；關於頁的示範資料說明修正（0e039d6）
- 2026-09-20 追加：新聞頁（N1，使用者決定）：左軟鍵選單 › 新聞 → 清單（本國最近 7 天、最多 9 則、提到我的地區的排前面）→ 內容（兩句 AI 摘要、發布者與網域、相關作物按 1 看行情；不開外部網站）；worker 每國當地 00:00 搜 Google 新聞，只收價格新聞，讀原文後用 Gemini 免費額度摘要（沒設 `GEMINI_API_KEY` 就只顯示標題），每天最多讀 30 篇、呼叫模型 30 次；新表 news_items、news_runs（migration 23655d2e487a）；make lint、make test（前端 495、後端 303）、make e2e（152 項）通過（6763b56）
- 2026-09-20 追加：新聞頁（N1，使用者決定）：左軟鍵選單 › 新聞 → 清單（本國最近 7 天、最多 9 則、提到我的地區的排前面）→ 內容（兩句 AI 摘要、發布者與網域、相關作物按 1 看行情；不開外部網站）；worker 每國當地 00:00 搜 Google 新聞，只收價格新聞，讀原文後用 Gemini 免費額度摘要（沒設 `GEMINI_API_KEY` 就只顯示標題），每天最多讀 30 篇、呼叫模型 30 次；新表 news_items、news_runs（migration 23655d2e487a，接在 B5 的 80ac1f2d0366 之後）；選單「新聞」排在「國際參考價」之後（首頁 7、詳情頁 8）；rebase 到含 B5 的 main 後 make lint、make test（前端 541、後端 405）、make e2e（170 項）通過（7172cd6）
- 2026-09-20 追加：新聞頁（N1，使用者決定）：左軟鍵選單 › 新聞（排在國際參考價之後：首頁 7、詳情頁 8）→ 清單（本國最近 7 天、最多 9 則、提到我的地區的排前面）→ 內容（兩句 AI 摘要、發布者與網域、相關作物按 1 看行情；不開外部網站）；worker 每國當地 00:00 搜 Google 新聞（台灣、印度、馬來西亞，馬來西亞含馬來文標題與別名），只收價格新聞，讀原文後用 Gemini 免費額度摘要（沒設 `GEMINI_API_KEY` 就只顯示標題），每天最多讀 30 篇、呼叫模型 30 次；新表 news_items、news_runs（migration 23655d2e487a，接在 74f6aadac0c6 之後）；rebase 到 fe0a814 後 make lint、make test（前端 550、後端 472）、make e2e（220 項）通過
- 2026-09-20 追加：馬來文（`ms`）與印地文（`hi`）介面（加分項 B6 的機器翻譯版，**待母語者檢查**）：兩個字串檔翻完 en.json 的每一個 key，缺 key 時執行中退回英文；`locales.test.ts` 只對 zh-TW／en 要求 key 完全相同，ms／hi 只印涵蓋率（目前各 244/244），別的分支加字串不會被擋。語言清單變成四個主要語言（手機語言第一；設定裡接著放該國的語言：印度 en、hi，馬來西亞 ms、en、zh-TW，台灣 zh-TW、en），हिन्दी 不再標「尚未提供」。數字仍依國家 locale 但一律拉丁數字，日期、星期、月份加上兩種語言，走勢圖改用字串檔的星期縮寫（天城文截字會壞）。天城文寫進字型堆疊，128×160 比照中文維持 11px。逐頁量過寬度後縮短放不下的字串（數字不再被截斷），軟鍵過長的標籤改成省略號、不再蓋住中間鍵；e2e 多了軟鍵重疊檢查與 ms／hi 的畫面、首次設定與主要流程。make lint、make test（前端 582、後端 472）、make e2e（380 項）全部通過
- 2026-09-20 追加：新聞兩處修正（使用者決定，見 decisions.md）：①清單一律依發布時間新到舊，不再把提到我的地區的排前面（提到的地區仍在 `area_ids`，資訊列照舊顯示我的地區）；②worker 的摘要額度改成只花在「API 會回的那份清單」上（最近 7 天最新的 9 則，同一個排序與上限，含前幾次存下來還沒有摘要的，重試上限仍是 2 次），新到舊一則一則做，排在清單之外的等它排進清單才輪到它——每日額度（各 30、滾動 24 小時、三國共用）與呼叫間隔不變，實際買到的是每國每天最多 9 則摘要（原本台灣存 24 則、讀 10 篇、只有 8 則摘要，且不一定落在清單上）；make lint、make test（前端 582、後端 475）、make e2e（380 項）通過
- 2026-09-20 追加：馬來西亞擴充到濕巴剎回報足夠的 75 個縣（規則：過去 60 天全國有資料的日子裡，超過一半的日子至少有 2 個濕巴剎回報；原本 11 個代號不變，追加的 64 個縣座標用 OpenStreetMap 查一次、`k` 由真實資料算），濕巴剎改在執行時從 `lookup_premise.csv` 找（seed 只列縣，新開的濕巴剎不必改 seed），國家可以在 seed 宣告預設價格類型（migration `5c1e7a9d2b40`；馬來西亞＝零售，選國家時套用，`*` 照樣切換）；新聞的州名別名改成該州每個縣都有；本機實跑真實資料 4 個請求（lookup 與 3 個月檔）約 30 秒、21.4 萬列對照得到、68,143 筆地區零售價，抽查 5 個縣的中位數與原始 CSV 相同，之後的更新 3 個請求全是 304；馬來西亞加上茄子（跨國比價要的作物；香蕉、芒果濕巴剎沒有資料，見 decisions）；rebase 到 main（672bdf3，含新聞與馬來文／印地文介面）後 make lint、make test（前端 584、後端 478）、make e2e（380 項）全部通過；預設仍是 mock，伺服器尚未開啟（feat/my-complete）
- 2026-09-20 追加：跨國比價（使用者要求，改掉 00 的「不做跨國比價」）：比價頁地區排名下面多一張「各國參考價」卡片，每個其他國家一行（國家、換算成使用者幣別與單位的全國價、批發或零售、地區數與交易日），全國價＝該國自己最新交易日各地區價的中位數；換算用 B5 的 `fx_rates` 走美元中轉並標匯率日期（取最舊的一天），缺價格或缺匯率顯示「—」與原因；有 Pink Sheet 序列的作物再加一行世界銀行世界價；其他國家都沒有這項作物時只寫一行說明。走現有的 `/crops/{crop}/compare`（新欄位 `other_countries`），沒有新端點、沒有新表
- 2026-09-20 追加：跨國比價（使用者要求，改掉 00 的「不做跨國比價」）：比價頁地區排名下面多一張「各國參考價」卡片，每個其他國家一行（國家、換算成使用者幣別與單位的全國價、批發或零售、地區數與交易日），全國價＝該國自己最新交易日各地區價的中位數；換算用 B5 的 `fx_rates` 走美元中轉並標匯率日期（取最舊的一天），缺價格或缺匯率顯示「—」與原因；有 Pink Sheet 序列的作物再加一行世界銀行世界價；其他國家都沒有這項作物時只寫一行說明。走現有的 `/crops/{crop}/compare`（新欄位 `other_countries`），沒有新端點、沒有新表；make lint、make test（前端 589、後端 502，services＋ingest 覆蓋率 99%）、make e2e（384 項）全部通過
- 2026-09-20 追加：台灣真實資料補齊＋各國自己的作物分類（使用者要求）：地區從 10 個縣市增為 13 個（加彰化、南投、台東）、市場從 14 個增為 19 個（農業部 FarmTransData 裡全部的果菜批發市場，各給大約座標，距離由座標算），作物依最近 60 天的實際覆蓋重選成 29 種（覆蓋率 53–90%，平均 71%），mock 的價格與區間改用真實中位數；分類改成每個國家自己定義（seed `country.categories`，最多 8 個，migration `5c3e1f7a9b20`），台灣是葉菜類、根莖類、瓜類、花果菜類、辛香料、水果，印度與馬來西亞沿用預設七個，首頁九宮格拿掉「全部」改成分類＋最近看過、名稱來自 API；新畫 8 個作物圖示。實跑 `PROVIDERS=mock,tw_moa`：29 個請求、約 50 秒、20,403 列、17,997 列入庫，抽查台北市甘藍、彰化縣包心白菜的地區價與原始資料的中位數相同。rebase 到 main（含 news）後 make lint、make test（前端 552、後端 484，覆蓋率 98%／核心 99%）、make e2e（234 項）全部通過
- 2026-09-20 追加：新聞移到首頁第三個分頁、國際參考價移到九宮格（使用者要求）：選單少了兩列，九宮格＝該國分類（最多 7 個）＋國際價＋最近，新聞畫面多了分頁列、選單與 `#`；四種語言都補上 `categories.intl`；台灣加回芒果（三國共同 12 種作物），共 30 種。rebase 到 main（含 ms/hi、馬來西亞 75 個縣、跨國參考價）後 make lint、make test（前端 592、後端 515）、make e2e（396 項）全部通過
- 2026-09-20 追加：B3 印度真實批發行情（`in_agmarknet`，Agmarknet 2.0，不需金鑰、GODL-India）：地區改成馬哈拉施特拉邦、卡納塔卡邦與德里 60 天內有回報的 64 個縣與 429 個市場（縣中心取自 OpenStreetMap），作物依真實涵蓋度並包含三國共同核心重選 21 種（新畫 3 個圖示），印度沒有免金鑰的零售來源所以真實資料的零售是「—」；一次執行 63 個請求（邦 × 品項 × 當月），每天 00:05 與 20:00 各一次（約 126 個請求／天），第一次抓 60 天＝189 個請求、約 6 分鐘；本機實跑 52,088 列、存 50,146 筆，抽查 4 個地區日的中位數與原始回應完全相同；make lint、make test（前端 534、後端 382）、make e2e 通過；預設仍是 mock
- 2026-09-20 追加：B3 印度真實批發行情（`in_agmarknet`，Agmarknet 2.0，不需金鑰、GODL-India）：地區改成馬哈拉施特拉邦、卡納塔卡邦與德里 60 天內有回報的 64 個縣與 429 個市場（縣中心取自 OpenStreetMap），作物依真實涵蓋度並包含三國共同核心重選 21 種（新畫 3 個圖示），印度沒有免金鑰的零售來源所以真實資料的零售是「—」；一次執行 63 個請求（邦 × 品項 × 當月），每天 00:05 與 20:00 各一次（約 126 個請求／天），第一次抓 60 天＝189 個請求、約 7 分鐘；本機實跑 49,837 列、存 47,914 筆，抽查 6 個地區日的中位數、市場數與高低和原始回應完全相同；之後 rebase 到含跨國比價與馬來西亞 75 縣的 main，附近價格改用 100 km 規則後印度多數縣沒有鄰近地區（Nashik 最近的 Dhule 是 108 km）；make lint、make test、make e2e 通過；預設仍是 mock
