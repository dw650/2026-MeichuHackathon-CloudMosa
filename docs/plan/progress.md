...
- 2026-09-20 追加：馬來西亞擴充到濕巴剎回報足夠的 75 個縣（規則：過去 60 天全國有資料的日子裡，超過一半的日子至少有 2 個濕巴剎回報；原本 11 個代號不變，追加的 64 個縣座標用 OpenStreetMap 查一次、`k` 由真實資料算），濕巴剎改在執行時從 `lookup_premise.csv` 找（seed 只列縣，新開的濕巴剎不必改 seed），國家可以在 seed 宣告預設價格類型（migration `5c1e7a9d2b40`；馬來西亞＝零售，選國家時套用，`*` 照樣切換）；新聞的州名別名改成該州每個縣都有；本機實跑真實資料 4 個請求（lookup 與 3 個月檔）約 30 秒、21.4 萬列對照得到、68,143 筆地區零售價，抽查 5 個縣的中位數與原始 CSV 相同，之後的更新 3 個請求全是 304；make lint、make test（前端 552、後端 478）、make e2e（220 項）全部通過；預設仍是 mock，伺服器尚未開啟（feat/my-complete）
