...
- 2026-09-20 追加：跨國比價（使用者要求，改掉 00 的「不做跨國比價」）：比價頁地區排名下面多一張「各國參考價」卡片，每個其他國家一行（國家、換算成使用者幣別與單位的全國價、批發或零售、地區數與交易日），全國價＝該國自己最新交易日各地區價的中位數；換算用 B5 的 `fx_rates` 走美元中轉並標匯率日期（取最舊的一天），缺價格或缺匯率顯示「—」與原因；有 Pink Sheet 序列的作物再加一行世界銀行世界價；其他國家都沒有這項作物時只寫一行說明。走現有的 `/crops/{crop}/compare`（新欄位 `other_countries`），沒有新端點、沒有新表；make lint、make test（前端 587、後端 496，services＋ingest 覆蓋率 99%）、make e2e（384 項）全部通過
