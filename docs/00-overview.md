...
- **2026-09-20 追加**（使用者要求）：比價頁多一張「各國參考價」卡片，做**跨國比價**（改掉原本「不做跨國比價」的決定），算法與換算見 [06](06-data.md) §4.1。
- **2026-09-20 追加**（使用者同意）：印度接上**真實批發行情**（Agmarknet 2.0，不需金鑰，GODL-India；`PROVIDERS` 加上 `in_agmarknet`，見 [06](06-data.md) §1.6）：地區改成三個邦的 64 個縣、429 個市場，作物依真實回報量並含三國共同核心重選 21 種；印度沒有免金鑰的零售來源，零售由批發推估並標示（[06 §3.6](06-data.md)）。
- **2026-09-20 追加**（使用者同意）：資料來源定下共同介面（每個來源宣告 `SourceInfo`、登記在 `registry.py`、共用抓取原則與契約測試，見 [06](06-data.md) §1.4）；新增第三個國家**馬來西亞**，可接官方 PriceCatcher 的真實零售價（`PROVIDERS` 加上 `my_pricecatcher`，見 [06](06-data.md) §1.5）。PriceCatcher 的批發市場 2026 年起沒有回報，所以馬來西亞只有零售價。同日擴充到**濕巴剎回報足夠的 75 個縣**（規則見 [06](06-data.md) §1.5），濕巴剎改在執行時從 PriceCatcher 的回報點對照表找；國家可以在 seed 宣告預設的價格類型，馬來西亞預設零售（使用者決定）。
- **2026-09-20 追加**（使用者要求）：來源缺的那一種價格改由另一種**推估**並標示，不再一律顯示「—」（[06 §3.6](06-data.md)）。
...
| 比價範圍 | 國內比較各地區；另外在比價頁加一張**各國參考價**卡片，列出其他國家同一作物的全國價與世界銀行的國際參考價，換算成使用者的幣別與單位（2026-09-20 追加，使用者要求；原本的決定是不做跨國比價） | [02](02-product-spec.md) |
| 顯示幣別 | 預設各國自己的幣別；使用者可在設定改成新台幣、馬幣、盧比或美元，**只換算顯示**（資料庫、API、比價仍是當地幣別），缺匯率就留在當地幣別（F19，2026-09-20 追加） | [02](02-product-spec.md) §5.7 |
| 示範國家 | 印度（英文、印地文、盧比；馬哈拉施特拉邦、卡納塔卡邦與德里共 64 個縣）、台灣（繁中、新台幣）與馬來西亞（英文、令吉；2026-09-20 追加，使用者同意） | [06](06-data.md) |
| 資料 | 先用 mock 資料，但一定經過後端 API 與資料庫；串接真實資料是加分項 | [06](06-data.md) |
| 新聞 | 每個國家一頁農業新聞，清單照發布時間新到舊；摘要由模型產生、用**該國自己的語言**（台灣繁中、印度印地文、馬來西亞馬來文；2026-09-20 改），語言不對、讀不到原文就只顯示標題，絕不編造；作物標籤寧缺勾不錯誤 | [06](06-data.md) |
| 推估價（2026-09-20 追加，使用者要求） | 某個國家的來源只提供一種價格時，另一種由它乘上固定倍率推估（倍率放在 `seed/derive.yaml`），只推估來源沒有的那一種；推估值不進 `quotes`、不覆蓋真實資料，而且**每個顯示它的畫面都標示成推估**（標籤「≈零售」＋一行說明） | [06 §3.6](06-data.md) |
...
| 不做 | 使用者輸入出價、夜間主題、到手價（扣運費估算）、賣方／買方角色、金流、帳號密碼、GPS、推播、離線、價格預測 | [02](02-product-spec.md) |
...
| 名詞 | 意思 |
|---|---|
| 國家 | 資料、幣別、單位的最大範圍。地區排名只在同一個國家內，跨國只在「各國參考價」卡片呈現 |
