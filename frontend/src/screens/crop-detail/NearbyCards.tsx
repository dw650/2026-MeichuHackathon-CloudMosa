...
import { type Nearby, NEARBY_SIDES, type NearbyRow } from './nearbyRows'
 ...
 * says so with 「你」 instead. Role-neutral: the same two rows serve sellers and buyers.
 */
export function NearbyCards({ nearby, fmt, keyCapOf }: NearbyCardsProps) {
  ...
  return NEARBY_SIDES.map(({ focusId, side }) => {
    const row = nearby[side]
    const label = t(`detail.today.nearby.${side}`)
    if (row.is_base) {
      return (
        <Card
          key={focusId}
    ...
    return (
      <Card
        ...
        name={name(row)}
        meta={`${label} · ${t('common.distance', { km: row.distance_km })}`}
