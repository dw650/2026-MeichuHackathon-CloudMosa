...
/**
 * `/settings/:item`: the list behind a settings row, 語言 (`language`), 顯示幣別 (`currency`)
 * or, in demo builds only, the Demo switches (`demo`). Anything else goes back to the
 * settings.
 */
export default function SettingsItemScreen() {
  const { item } = useParams()
  if (item === 'language') return <LanguageSettings />
  if (item === 'currency') return <CurrencySettings />
